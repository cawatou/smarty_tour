            ...
            <td style="vertical-align: top;" class="departure-dates-text">
                ...
                <div id="c-tour-price-fon" style="display: none;">
                    ...
                    {if $tour->getDiscountPercent() !== null}
                        <div id="c-tour-price">от {$price|escape|price_format} руб.</div>
                        <div id="c-tour-price-old">{$tour->getPrice()|escape|price_format} руб.</div>
                        <div id="c-tour-price-line"></div>
                    {else}
                        <div class="c-tour-price-old print-unstrike">от {$price|escape|price_format} руб.</div>
                    {/if}
                </div>
                ...
                {if $tour->getPayableExcludes() !== null && count($tour->getPayableExcludes()|array_filter) || count($tour->getGetVia())}
                    <div class="tour-payable-part">
                        <b>Оплачивается дополнительно:</b>

                        <ul>
                            {foreach $tour->getPayableExcludes() as $exclude}
                                <li>
                            ...
                            {if count($tour->getGetVia())}
                                <li>
                                    <span class="check-mark check-mark-gray">&check;</span>
                                    Проезд на одного человека

                                    <ul class="get-via-list">
                                        {foreach $tour->getGetVia() as $get_via}
                                            {if empty($get_via.price)}{continue}{/if}

                                            <li>
                                                <span class="get-via-title">{$get_via.title|escape}</span>

                                                <span class="get-via-price">
                                                    {$get_via.price}&nbsp;<i class="currency-rub icon-rub-red">i</i>
                                                </span>
                                            </li>
                                        {/foreach}
                                    </ul>
...
{***
{if count($tour->getGetVia()) > 0}
<div class="tour-get-via">
    <header class="tour-get-via-header">
        Стоимость проезда до курорта
        <small>на одного человека</small>
    </header>

    {foreach $tour->getGetVia() as $get_via}
    <label class="tour-get-via-item">
        <span class="tour-get-via-item-text">{$get_via.title|escape}</span>:
        {$get_via.price|price_format:false}
        <i class="currency-rub icon-rub-red">i</i>
        <input type="radio" class="tour-get-via-input" name="tour_get_via" value="{(float)$get_via.price}"{if $get_via@first} checked{/if} style="display: none;">
    </label>
    {/foreach}
</div>
{/if}
                        ...
                        {if $tour->getIsDiscountApplied() && !empty($departure_hotels[$k_h].is_discountable)}
                            ...
                            {if !empty($discounts[$type])}
                                {foreach $discounts[$type] as $discount}
                                    {$isFitting = $tour->isDiscountFitting($discount, $hotel_price, $tour)}
