...
;(function ($, window, document, undefined) {
    var $window = $(window);
    var defaults = {
        syntax: '<div class="selectbox-link"><div class="selectbox-link-text"></div></div><div class="selectbox-dropdown"><div class="selectbox-list"></div></div>',
    ...
    $.extend(IkSelect.prototype, {
        init: function () {
            this.$wrapper = $('<div class="selectbox">' + this.options.syntax + '</div>'); // wrapper for the fake select and the fake dropdown
            this.$link = $('.selectbox-link', this.$wrapper); // fake select
            this.$linkText = $('.selectbox-link-text', this.$wrapper); // fake select's text
            this.$dropdown = $('.selectbox-dropdown', this.$wrapper); // fake dropdown
            this.$list = $('.selectbox-list', this.$wrapper); // options list inside the fake dropdown
            this.$listInner = $('<div class="selectbox-list-inner"/>'); // support dropdown for scrolling
            this.$active = $([]); // active fake option
            this.$hover = $([]); // hovered fake option
            this.hoverIndex = 0; // hovered fake option's index

            this.dropdownAddedWidth = 0;
            ...
            if (this.options.filter) {
                ...
                this.$nothingFoundText = $('<div class="selectbox-nothing-found"/>').html(this.options.nothingFoundText);
                this.$filterWrap = $('.selectbox-filter-wrap', this.$wrapper);

                if (! this.$filterWrap.length) {
                    this.$filterWrap = $('<div class="selectbox-filter-wrap"/>');
                }

                this.$filter = $('<input type="text" class="selectbox-filter">');
        ...
        _optionMouseover: function (event) {
            ...
            if ($option.hasClass('selectbox-option-disabled')) {
                return;
            }
            this.$hover.removeClass('selectbox-hover');
            this.$hover = $option.addClass('selectbox-hover');
        ...
        _makeOptionActive: function (index, shouldSync) {
            ...
            this.$link.toggleClass('selectbox-link-novalue', !$option.attr('value'));

            this.$hover.removeClass('selectbox-hover');
            this.$active.removeClass('selectbox-active');
            this.$hover = this.$active = this.$optionSet.eq(index).addClass('selectbox-hover selectbox-active');
        ...
        _moveTo: function (index) {
            ...
            if (!this.$dropdown.is(':visible') || $.browser.mozilla) {
                ...
                this.$hover.removeClass('selectbox-hover');
                this.$hover = this.$optionSet.eq(index).addClass('selectbox-hover');
                this.hoverIndex = index;
            }
            ...
            if (!this.$hover.index()) {
                $optgroup = this.$hover.closest('.selectbox-optgroup');
                if ($optgroup.length) {
        ...
        _moveToFirstActive: function () {
            for (var i = 0; i < this.$optionSet.length; i++) {
                if (!this.$optionSet.eq(i).hasClass('selectbox-option-disabled')) {
                    this._moveTo(i);
                    break;
                }
            }
        ...
        _moveToPrevActive: function (condition) {
            ...
            for (var i = this.hoverIndex - 1; i >= 0; i--) {
                ...
                if (!$option.hasClass('selectbox-option-disabled') && (typeof condition === 'undefined' || condition($option.position().top))) {
                    this._moveTo(i);
                    break;
                }
            }
        ...
        _elFocus:  function () {
            ...
            this.$link.addClass('selectbox-link-focus');
        ...
        // when focus lost remove the focus class from the fake one
        _elBlur: function () {
            this.$link.removeClass('selectbox-link-focus');
        },
                ...
                if (filterVal === '') {
                    ...
                    if (this.$optionSet.length) {
                        ...
                        this.$optgroupSet.each(function (optgroupIndex, optgroup) {
                            ...
                            if (!$('.selectbox-option:visible', $optgroup).length) {
                                $optgroup.hide();
                            }
                        });
        ...
        _fixHeight: function () {
            ...
            this.$listInner.find('.selectbox-option').each(function () {
                _h += $(this).height();
            });

            var that = this;

            if (_h > that.options.ddMaxHeight) {
                that.$listInner.css({
                    overflow: 'auto',
                    height: that.options.ddMaxHeight,
                    position: 'relative'
                });
            }
        ...
        redraw: function () {
            ...
            if (this.options.dynamicWidth || this.options.autoWidth || this.options.ddFullWidth) {
                ...
                if (this.options.dynamicWidth) {
                    ...
                    this.$wrapper.width(maxWidthOuter + (this.options.equalWidths ? scrollbarWidth : 0)).addClass('selectbox-autowidth');
                }
        ...
        reset: function () {
            ...
            this.$optgroupSet = $('.selectbox-optgroup', this.$listInner);
            this.$optionSet = $('.selectbox-option', this.$listInner);
        },
        ...
        hideDropdown: function () {
            ...
            this.$link.removeClass('selectbox-link-dropdown-opened');

            if (this.options.extractLink) {
                this.$wrapper.outerWidth(this.$wrapper.data('outerWidth'));
                this.$wrapper.height('');
                this.$link.removeClass('selectbox-link-extracted').css({
                    position: '',
                    top: '',
                    left: '',
                    zIndex: ''
        ...
        showDropdown: function () {
            ...
            this.$link.addClass('selectbox-link-dropdown-opened');
            ...
            this.$dropdown.css({
                ...
                top:   dropdownOffset.top - 2,
                width: this.$dropdown.width() - this.dropdownAddedWidth + 28
            }).appendTo('body');

            this.dropdownAddedWidth = 28;
            ...
            if (this.options.extractLink) {
                ...
                this.$link.addClass('selectbox-link-extracted').css({
                    position: 'absolute',
                    top: linkOffset.top,
        ...
        _generateOptionObject: function ($option) {
            return {
                value: $option.val(),
                label: $option.attr('data-html') || $option.html() || '&nbsp;',
        ...
        _renderListOption: function (option) {
            var html;
            var disabledClass = option.isDisabled ? ' selectbox-option-disabled' : '';
            html = '<li class="selectbox-option' + disabledClass + '" data-value="' + option.value + '">';
            html += '<span class="selectbox-option-label">';
            html += option.label;
            html += '</span>';
            html += '</li>';
            return html;
        ...
        _renderListOptgroup: function (optgroup) {
            var html;
            var disabledClass = optgroup.isDisabled ? ' selectbox-optgroup-disabled' : '';
            html = '<li class="selectbox-optgroup' + disabledClass + '">';
            html += '<div class="selectbox-optgroup-label">' + optgroup.label + '</div>';
            html += '<ul>';
        ...
        addOptions: function (options, optionIndex, optgroupIndex) {
            ...
            if ($.isNumeric(optionIndex)) {
                $ulOptions = $('.selectbox-option', $destinationUl);
        ...
        removeOptions: function (optionIndexes, optgroupIndex) {
            ...
            if ($.isNumeric(optgroupIndex)) {
                if (optgroupIndex < 0) {
                    $listContext = $('> .selectbox-option', this.$listInnerUl);
                    $selectContext = $('> option', this.$el);
                } else if (optgroupIndex < this.$optgroupSet.length) {
                    $listContext = $('.selectbox-option', this.$optgroupSet.eq(optgroupIndex));
        ...
        toggle: function (bool) {
            ...
            this.$link.toggleClass('selectbox-link-disabled', this.isDisabled);
        },
        ...
        toggleOptgroups: function (optgroupIndexes, bool) {
            ...
            $.each(optgroupIndexes, $.proxy(function (index, optgroupIndex) {
                ...
                this.$optgroupSet.eq(optgroupIndex).toggleClass('selectbox-optgroup-disabled', !isDisabled);
        ...
        toggleOptions: function (lookupValues, isIndex, bool) {
            ...
            var toggleOption = $.proxy(function ($option, optionIndex) {
                ...
                this.$optionSet.eq(optionIndex).toggleClass('selectbox-option-disabled', !isDisabled);
            }, this);
