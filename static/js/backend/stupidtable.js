    ...
    return this.each(function() {
      ...
      sortFns = $.extend({}, {
        ...
        "fls-string-ins": function(a, b) {
          ...
          a = a[1].toString().toLowerCase();
          b = b[1].toString().toLowerCase();
          if (a < b) return -1;
          if (a > b) return +1;
          return 0;
        }
      ...
      var sort_map = function(arr, sort_function, reverse_column) {
        ...
        var sort_map_fs = function(arr, fs, sort_function, reverse_column) {
            var map = [];
            if (reverse_column) {
                var sorted = arr.slice(0).reverse(sort_function);
            } else {
                var sorted = arr.slice(0).sort(sort_function);
            }

            var dirs = [], files = [], index = 0;;
            for (var i=0; i<sorted.length; i++) {
                index = $.inArray(sorted[i], arr)
                if (fs[index] == 'dir') {
                    dirs.push(sorted[i]);
                } else {
                    files.push(sorted[i]);
      ...
      $table.on("click", "th", function() {
        ...
        var is_fs = $this.data('fs') ? true : false;
        ...
        setTimeout(function() {
          ...
          trs.each(function(index,tr) {
            ...
            if (is_fs) {
                fs_column.push($(tr).data('fs-value'));
            }
          });
          ...
          if (is_fs) {
              var theMap = sort_map_fs(column, fs_column, sortMethod, reverse_column);
          } else {
